...
// ---------------------------------------------------------------------------
// Fragment shader — Halftone (verbatim from spec)
// ---------------------------------------------------------------------------
const FS = `
...
const SHAPE     = new Float32Array([1.48, 0.88, 0.50, 0.00])
const SURFACE   = new Float32Array([2.40, 0.91, 0.00, 1.00])
const FINISH    = new Float32Array([0.00, 0.00, 0.00, 0.06])
const TRANSFORM = new Float32Array([2926.0, 0.00, 0.00, 0.0])
const SPACE     = new Float32Array([0.00, 0.00, 0.00, 0.00])
const CURSOR    = new Float32Array([0.00, 2.00, 0.65, 0.46])
...
export default function ShaderCanvas() {
  ...
  useEffect(() => {
    ...
    gl.uniform3fv(uColors, COLORS_FLAT)
    gl.uniform4fv(uShape,     SHAPE)
    gl.uniform4fv(uSurface,   SURFACE)
    gl.uniform4fv(uFinish,    FINISH)
    gl.uniform4fv(uTransform, TRANSFORM)
    gl.uniform4fv(uSpace,     SPACE)
    gl.uniform4fv(uCursor,    CURSOR)
    ...
    function render(now: number) {
      ...
      elapsed += (now - lastTs) * 0.001
      lastTs = now
      gl!.uniform4f(uScene, W, H, elapsed * 1.43, 4.0)
